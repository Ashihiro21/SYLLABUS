...
if(isset($_POST['updatedata'])) {
    // Get the form data
    $update_id45 = $_POST['update_id45'];

    // Handle the file upload
    $uploadOk = 1;
    $imageFileType = strtolower(pathinfo($_FILES["commitee_signature"]["name"], PATHINFO_EXTENSION));
    ...
    if (move_uploaded_file($_FILES["commitee_signature"]["tmp_name"], $uploadDirectory . basename($_FILES["commitee_signature"]["name"]))) {
        // File uploaded successfully, now update the database

        // Get the file name without the directory
        $filename = basename($_FILES["commitee_signature"]["name"]);

        // Add the directory to the filename
        $filenameWithPath = $postDirectory . $filename;

        // SQL query to update the database
        $sql = "UPDATE course SET commitee_signature = '$filenameWithPath' WHERE id = $update_id45";

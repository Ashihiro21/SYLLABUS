...
EOH;
    ...
    public function createDoc($html, $file, $download = false)
    {
        ...
        $doc = $this->getHeader() . $this->htmlBody . $this->getFooter();
     ...
     // Fetch content from database and replace placeholders in HTML
     public function createDocFromDatabase($templateFile, $data, $file, $download = false)
     {
         ...
         $this->setDocFileName($file);
         $doc = $this->getHeader() . $html . $this->getFooter();
    ...
    private function _parseHtml($html)
    {
        ...
        // Add code to extract and embed images here
        $html = $this->embedImages($html);
    ...
    private function embedImages($html)
    {
        ...
        // Loop through each image and embed it into the HTML
        foreach ($images as $image) {
            // Get the image data and encode it
            $imageData = base64_encode(file_get_contents($image));
            // Replace the image URL with base64 data
            $html = str_replace($image, 'data:image/jpeg;base64,' . $imageData, $html);
        }

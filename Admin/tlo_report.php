...
list($midtermHigherResults, $midtermHigherPercent) = filterLearningOutcomes($conn, $higherArray, 'course_leaning', 'topic_learn_out');
list($midtermLowerResults, $midtermLowerPercent) = filterLearningOutcomes($conn, $lowerArray, 'course_leaning', 'topic_learn_out');

// Get filtered results for final period
list($finalHigherResults, $finalHigherPercent) = filterLearningOutcomes($conn, $higherArray, 'laerning_final', 'final_topic_leaning_out');
list($finalLowerResults, $finalLowerPercent) = filterLearningOutcomes($conn, $lowerArray, 'laerning_final', 'final_topic_leaning_out');
        ...
        <?php if (!empty($midtermHigherResults[$department])): ?>
            <?php foreach ($midtermHigherResults[$department] as $result): ?>
                ...
                <table>
                    ...
                    <tr>
                        <td>
                            <p><strong>Outcome:</strong> <?php echo $result['comlab'] . '. ' . $result['topic_learn_out']; ?></p>
                        </td>
                        <td>
                            <?php
                            if (!empty($midtermLowerResults[$department])) {
                                foreach ($midtermLowerResults[$department] as $lowerResult) {
                                    if ($lowerResult['cname'] == $result['cname']) {
                                        echo '<p><strong>Outcome:</strong> ' . $lowerResult['comlab'] . '. ' . $lowerResult['topic_learn_out'] . '</p>';
                                    }
                                }
                            } else {
        ...
        <?php if (!empty($finalHigherResults[$department])): ?>
            <?php foreach ($finalHigherResults[$department] as $result): ?>
                ...
                <table>
                    ...
                    <tr>
                        <td>
                            <p><strong>Outcome:</strong> <?php echo $result['comlab'] . '. ' . $result['final_topic_leaning_out']; ?></p>
                        </td>
                        <td>
                            <?php
                            if (!empty($finalLowerResults[$department])) {
                                foreach ($finalLowerResults[$department] as $lowerResult) {
                                    if ($lowerResult['cname'] == $result['cname']) {
                                        echo '<p><strong>Outcome:</strong> ' . $lowerResult['comlab'] . '. ' . $lowerResult['final_topic_leaning_out'] . '</p>';
                                    }
                                }
                            } else {
                                echo '<p>No Lower Level found.</p>';
